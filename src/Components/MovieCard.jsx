        ...
        <div className="card" style={{ width: "300px", margin: "10px", padding: "10px" }}>
            ...
            {isDetail && (
                ...
                <div className="reviews-section mt-4 text-start">
                    <h3 className="border-bottom pb-2">Recensioni degli utenti</h3>
                    {!movie.reviews || movie.reviews?.length === 0 ? (
                        <p className="text-muted">Nessuna recensione ancora presente per questo film.</p>
                    ) : (
                        <div className="d-flex flex-column gap-3 mt-3">
                            {movie.reviews.map((review, index) => (
                                <div key={index} className="card" style={{ padding: "15px", backgroundColor: "#f9f9f9" }}>
                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                        <h5 className="m-0"><strong>{review.name}</strong></h5>
                                        <span className="badge bg-warning text-dark">
                                            Voto: {review.vote} / 5 ⭐
                                        </span>
                                    </div>
                                    <p className="mb-0 text-secondary" style={{ fontStyle: "italic" }}>
                                        "{review.text}"
                                    </p>
                                </div>
                            ))}
